...
type PlaygroundProps = {
	/**
	 * Server-rendered option reference, handed down as an already-rendered
	 * element from the server page. It must be a ReactNode, not a component —
	 * a function cannot cross the RSC boundary.
	 */
	optionReference?: React.ReactNode;
};

export default function Playground({ optionReference }: PlaygroundProps) {
	...
	const optionsSurface = (
		<div className="flex h-full flex-col">
			...
			<div className="flex-1 overflow-auto p-2">
				...
				{/* Never gate this on isLargeScreen: that flag starts false and is only
				    set in an effect, so anything behind it renders in zero server HTML
				    and would defeat the entire point of server-rendering it. */}
				{optionReference}
